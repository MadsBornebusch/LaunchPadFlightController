...
//
//*****************************************************************************
static unsigned char g_pcUARTTxBuffer[UART_TX_BUFFER_SIZE2];
...
//
//*****************************************************************************
static unsigned char g_pcUARTRxBuffer[UART_RX_BUFFER_SIZE2];
...
//
//*****************************************************************************
#define TX_BUFFER_USED2          (GetBufferCount(&g_ui32UARTTxReadIndex,  \
                                                &g_ui32UARTTxWriteIndex, \
                                                UART_TX_BUFFER_SIZE2))
#define TX_BUFFER_FREE2          (UART_TX_BUFFER_SIZE2 - TX_BUFFER_USED2)
#define TX_BUFFER_EMPTY2         (IsBufferEmpty(&g_ui32UARTTxReadIndex,   \
                                               &g_ui32UARTTxWriteIndex))
...
//
//*****************************************************************************
#define RX_BUFFER_USED2          (GetBufferCount(&g_ui32UARTRxReadIndex,  \
                                                &g_ui32UARTRxWriteIndex, \
                                                UART_RX_BUFFER_SIZE2))
#define RX_BUFFER_FREE2          (UART_RX_BUFFER_SIZE - RX_BUFFER_USED2)
#define RX_BUFFER_EMPTY2         (IsBufferEmpty(&g_ui32UARTRxReadIndex,   \
                                               &g_ui32UARTRxWriteIndex))
#define RX_BUFFER_FULL2          (IsBufferFull(&g_ui32UARTRxReadIndex,  \
                                              &g_ui32UARTRxWriteIndex, \
                                              UART_RX_BUFFER_SIZE2))
...
UARTPrimeTransmit(uint32_t ui32Base)
{
    ...
    if(!TX_BUFFER_EMPTY2)
    {
        ...
        while(MAP_UARTSpaceAvail(ui32Base) && !TX_BUFFER_EMPTY2)
        {
            ...
            ADVANCE_TX_BUFFER_INDEX2(g_ui32UARTTxReadIndex);
        }
    ...
    for(uIdx = 0; uIdx < ui32Len; uIdx++)
    {
        ...
        if(pcBuf[uIdx] == '\n')
        {
            if(!TX_BUFFER_FULL)
            {
                ...
                ADVANCE_TX_BUFFER_INDEX2(g_ui32UARTTxWriteIndex);
            }
            else
            {
                //
...
UARTgets2(char *pcBuf, uint32_t ui32Len)
{
    ...
    while(1)
    {
        ...
        if(!RX_BUFFER_EMPTY2)
        {
            ...
            ADVANCE_RX_BUFFER_INDEX2(g_ui32UARTRxReadIndex);
...
//*****************************************************************************
void
UARTvprintf2(const char *pcString, va_list vaArgP)
{
...
//! buffered mode using \b UART_BUFFERED2, may be used to determine the number
...
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED2, may be used to determine the amount
...
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED2, may be used to look ahead in the
...
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED2, may be used to discard any data
...
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED2, may be used to flush the transmit
...
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED2, may be used to control whether or not
    ...
    if(ui32Ints & (UART_INT_RX | UART_INT_RT))
    {
        ...
        while(MAP_UARTCharsAvail(g_ui32Base))
        {
            ...
            if(!g_bDisableEcho)
            {
                ...
                if(cChar == '\b')
                {
                    ...
                    if(!RX_BUFFER_EMPTY2)
                    {
                        ...
                        {
                            g_ui32UARTRxWriteIndex = UART_RX_BUFFER_SIZE2 - 1;
                        }
                        else
                        {
                            g_ui32UARTRxWriteIndex--;
            ...
            if(!RX_BUFFER_FULL2)
            {
                ...
                ADVANCE_RX_BUFFER_INDEX2(g_ui32UARTRxWriteIndex);
